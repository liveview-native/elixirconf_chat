    ...
    {~T[08:40:00.000], ~T[09:40:00.000],
     [
       {"Keynote", ["Katelynn Burns"],
        [subtitle: "Motion Commotion: Motion Tracking with Bumblebee and Liveview"]}
     ]},
...
for {day, timeslots} <- schedule do
  for {from, to, talks} <- timeslots do
    ...
    timeslot =
      ...
      |> Timeslot.changeset(%{
        starts_at:
          DateTime.new!(day, from, "EST")
          |> DateTime.shift_zone!("UTC")
          |> DateTime.to_naive()
          |> NaiveDateTime.truncate(:second),
        ends_at:
          DateTime.new!(day, to, "EST")
          |> DateTime.shift_zone!("UTC")
          |> DateTime.to_naive()
          |> NaiveDateTime.truncate(:second),
        timezone: "EST"
